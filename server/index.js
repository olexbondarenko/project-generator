...
const generateProject = async (req, res) => {
  ...
  const globalFolderPath = `${__dirname}/${globalName}`;
  ...
  try { 
    ...
    await createDirectory(`${globalFolderPath}/styles/helpers`)
    .then(async () => {
    ...
    for(let i = 0; i < components.length; i++) {
      ...
      if(templateName === 'default' || templateName === 'mainModal') {
        ...
        // create style files
        await createDirectory(`${globalFolderPath}/components/styles/${toCamelCase(`${globalName} ${componentName}`)}`)
        .catch((err) => {console.log(err)});
    ...
    if(useBanner) {
      await createDirectory(`${globalFolderPath}/components/banners/`)
      .then(async () => {
        await createFile(
      ...
      await createDirectory(`${globalFolderPath}/components/banners/styles`)
      .then(async () => {
        await createFile(
